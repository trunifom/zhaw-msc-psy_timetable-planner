...
- **Kritische Ersttermin-Konflikte** (🎓, nur bei hochgeladener Zusatzliste): Bachelor- und Mastermodule überschneiden sich bei Passerelle-Studierenden besonders oft - ein Konflikt ausgerechnet im 1. oder 2. Termin eines Kurses ist aber deutlich riskanter als später, weil dort meist einmalige Informationen zu Leistungsnachweisen, Prüfungsmodalitäten, Gruppenbildung, Ablauf und Literatur vermittelt werden. Das Dashboard zeigt dazu einen kurzen Hinweis/Zähler, die volle Detailtabelle (Datum, beide Kurse, welche Seite betroffen ist) steht im Konfliktanalyse-Tab. Erkennt sowohl Bachelor↔Master- als auch Bachelor↔Bachelor-Konflikte; reine Master↔Master-Konflikte gehören zur normalen Konfliktanalyse und werden hier nicht separat aufgeführt. Die Erkennungslogik (`find_critical_first_session_conflicts()` in `src/scheduler.py`) ist - anders als das meiste in `src/app.py` - vollständig mit pytest automatisiert getestet, siehe [docs/TESTING-README.md](docs/TESTING-README.md).
  - An beiden Stellen (Dashboard und Konfliktanalyse-Tab) gibt es zusätzlich ein einklappbares **"ℹ️ Warum ist das wichtig - und was tun?"**-Feld mit ausführlichem Hintergrund (was typischerweise in der 1./2. Sitzung vermittelt wird: Leistungsnachweise, Prüfungsmodalitäten, Gruppenbildung, Ablauf, Literaturliste) und konkreten Handlungsempfehlungen bei einem gefundenen Konflikt (Dozierende beider Kurse frühzeitig kontaktieren, Kommiliton:innen um Unterlagen bitten, nach Aufzeichnung/Ausweichtermin fragen, die Liste schon bei der Kurswahl in Schritt 2 berücksichtigen). Derselbe Text an beiden Stellen (`passerelle_critical.*`-Keys in `src/i18n.py`, nicht unter `dashboard.*`/`conflicts.*` verschachtelt) - eine Quelle, keine Drift zwischen Dashboard und Konfliktanalyse-Tab. In allen drei Sprachen (de/en/fr) vorhanden.

**Vorbereitet, aktuell deaktiviert:** Von den insgesamt sehr vielen Bachelor-Kursen dürfen Passerelle-Studierende laut ZHAW künftig nur einen kleinen, offiziell festgelegten Teil wählen - diese Liste existiert heute noch nicht. Damit sie später nicht mehr komplett neu entworfen werden muss, gibt es bereits eine Vorfilter-Funktion in `src/data_loader.py` (`load_zusatzmodule_allowlist()`/`_filter_zusatzmodule_by_allowlist()`), die Zeilen der Zusatzliste anhand einer Modul-Nr/Kurs-Nr-Liste aus einer JSON-Settingdatei im Ordner `settings/` ein- bzw. ausblendet. Das Feature ist über die Konstante `ZUSATZMODULE_ALLOWLIST_ENABLED` (Default `False`) komplett abschaltbar und hat in diesem Zustand keinerlei Wirkung; `settings/zusatzmodule_allowlist.json` enthält aktuell nur eine fiktive Platzhalterliste als Struktur-Beispiel. Sobald die offizielle ZHAW-Liste vorliegt, genügt es, die echten Modul-/Kurs-Nummern in diese Datei einzutragen und das Flag umzuschalten.
