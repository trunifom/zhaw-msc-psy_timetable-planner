...
| `tests/test_zusatzmodule_fixtures.py` | The same full import pipeline **at realistic real-world scale**, against the large-scale/scenario fixtures described in "Test data" below (full-size fictionalized Bachelor/Master catalogs, an engineered cross-catalog conflict pair, an engineered Modul-Nr collision pair): row counts surviving import, weekday derivation across the *entire* real date range, every real `Modulart` value, both real "parallel offerings" patterns (labeled `- Gruppe a..h` and unlabeled/ambiguous - see `app._has_undistinguished_parallel_offerings`), `ist_zusatzmodul` tagging, cross-catalog conflict detection end-to-end (not hand-built `ZHAWModule`s), the Modul-Nr-collision data-layer contract, Excel/ICS export at ~1500 combined rows, and (skipped unless `data/real/` is present locally) a direct diff against the real files proving no real name/module code/title leaked into the committed fixtures. | Integration-style, built on module-scoped fixtures loading the six fixtures generated by `tests/fixtures/generate_fictional_fixtures.py`. |
